...
<body>
  ...
  <div id="login" class="section active">
    <div class="form-container">
      ...
      <input type="text" id="codigo-filial" placeholder="Digite o código da filial" />
  ...
  <script>
    ...
    function entrar() {
      const codigo = document.getElementById('codigo-filial').value.trim();
      
      if (codigosFiliais[codigo]) {
    ...
    // Função para pré-selecionar a filial nos formulários
    ...
    function mostrarSecao(secaoId) {
      ...
      // Pré-selecionar a filial quando mostrar qualquer formulário
      if (secaoId !== 'login' && secaoId !== 'home') {
        preencherFilialAutomaticamente();
      }
      
      // Mostrar botão de voltar (exceto para login)
      if (secaoId !== 'login') {
    ...
    window.addEventListener('load', function() {
      const filialSalva = localStorage.getItem('filial');
      if (filialSalva) {
    ...
    async function inicializarScannerDynamsoft() {
      try {
        // Configura a licença do Dynamsoft (substitua pela sua chave de licença)
    ...
    function consultarNotaNF() {
      ...
      fetch(`${URL_SCRIPT_NF}?chave=${chave}&filial=${filialLogada.codigo}`)
        .then(res => res.json())
        .then(data => {
          if (data.success) {
